...
export default function FeatureCard({ feature, onOpen }: FeatureCardProps) {
  ...
  // ── XL hero tile (2×2): rich, with overview + highlights peek ──
  if (layout === "xl") {
    return (
      <button
        type="button"
        ...
        className={`${SHELL} ${SPAN.xl} p-7 sm:p-8`}
      >
        <div
          className={`pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full bg-gradient-to-br ${accent.glow} to-transparent opacity-0 blur-2xl transition-opacity duration-500 group-hover:opacity-100`}
        />
        <span
          className={`relative mb-5 flex h-14 w-14 items-center justify-center rounded-2xl transition-all duration-300 ease-out group-hover:scale-110 group-hover:-rotate-3 ${accent.badge}`}
        >
          <Icon className={`h-7 w-7 ${accent.icon}`} strokeWidth={1.8} />
        </span>
        <h3 className="relative mb-2.5 text-xl font-bold text-navy sm:text-2xl">
          {feature.title}
        </h3>
        <p className="relative mb-5 text-sm leading-relaxed text-ink-soft">
          {feature.overview}
        </p>
        <ul className="relative mb-6 grid gap-2 sm:grid-cols-2">
  ...
  // ── Wide banner tile (2×1): icon left, content right ──
  if (layout === "wide") {
    return (
      <button
        type="button"
        onClick={() => onOpen(feature)}
        aria-label={`View details for ${feature.title}`}
        className={`${SHELL} ${SPAN.wide} flex-row items-start gap-5 p-6 sm:p-7`}
      >
        <div
          className={`pointer-events-none absolute -right-16 -top-16 h-40 w-40 rounded-full bg-gradient-to-br ${accent.glow} to-transparent opacity-0 blur-2xl transition-opacity duration-500 group-hover:opacity-100`}
        />
        <span
          className={`relative flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl transition-all duration-300 ease-out group-hover:scale-110 group-hover:-rotate-3 ${accent.badge}`}
        >
          <Icon className={`h-7 w-7 ${accent.icon}`} strokeWidth={1.8} />
        </span>
        <div className="relative flex h-full flex-col">
          <h3 className="mb-1.5 text-lg font-bold text-navy sm:text-xl">
            {feature.title}
          </h3>
